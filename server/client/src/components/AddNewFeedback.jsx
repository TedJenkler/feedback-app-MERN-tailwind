            ...
            <form onSubmit={handleSubmit} className='flex flex-col text-black mt-5 py-11 px-6 bg-white rounded-xl md:py-14 md:px-12'>
                ...
                <div className='mb-6'>
                    ...
                    <input
                        ...
                        className={errors.title ? 'w-full px-13 bg-grey-white2 h-12 p-4 mb-1 focus:outline-strong-blue border-2 border-red rounded-xl md:px-15' : 'w-full px-13 bg-grey-white2 h-12 p-4 focus:outline-strong-blue mb-1 rounded-xl md:px-15'}
                        placeholder='Add a title'
                    />
                    {errors.title && <p className='text-red px-13 md:text-sm'>Can’t be empty</p>}
                </div>
                <CategorySelect
                ...
                <div className='mb-6'>
                    ...
                    <textarea
                        ...
                        onChange={handleInputChange}
                        className={errors.description ? 'w-full px-13 bg-grey-white2 h-32 p-3 focus:outline-strong-blue border-2 border-red rounded-xl md:px-15' : 'w-full px-13 bg-grey-white2 h-32 p-3 focus:outline-strong-blue rounded-xl md:px-15'}
                        placeholder='Add details'
                    ></textarea>
                    {errors.description && <p className='text-red px-13 md:text-sm'>Can’t be empty</p>}
                </div>
                <div className='flex flex-col md:flex-row-reverse md:gap-4 md:items-center'>
                    <button type="submit" className='px-13 bg-purple text-center text-white mb-4 py-2 rounded-xl font-bold text-sm md:py-2 md:mb-0 md:px-5 hover:bg-hover-purple md:text-sm'>Add Feedback</button>
                    <Link to="/feedback-app-tailwind-vite/" className='px-13 bg-blue text-white py-2 rounded-xl text-center text-sm font-bold md:py-2 md:mb-0 md:px-5 hover:bg-hover-grey md:text-sm'>Cancel</Link>
                </div>
            </form>
        </main>
    );
